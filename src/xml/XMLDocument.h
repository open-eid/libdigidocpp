...
struct unique_xml<D(T *)>
{
    using type = std::unique_ptr<T,D(*)(T *)>;
};

template<typename D>
using unique_xml_t = typename unique_xml<D>::type;

template<class T, typename D>
constexpr std::unique_ptr<T, D> make_unique_xml(T *p, D d) noexcept
{
    return {p, d};
}
...
struct XMLDocument: public unique_xml_t<decltype(xmlFreeDoc)>, public XMLNode
{
    ...
    bool validateSchema(const std::string &schemaPath) noexcept
    {
        auto schema_parser = make_unique_xml(xmlSchemaNewParserCtxt(schemaPath.c_str()), xmlSchemaFreeParserCtxt);
        //xmlSchemaSetParserErrors(schema_parser.get() ...)
        auto schema = make_unique_xml(xmlSchemaParse(schema_parser.get()), xmlSchemaFree);
        auto schema_validate = make_unique_xml(xmlSchemaNewValidCtxt(schema.get()), xmlSchemaFreeValidCtxt);
        xmlSchemaSetValidErrors(schema_validate.get(), schemaValidationError, schemaValidationWarning, nullptr);
        return xmlSchemaValidateDoc(schema_validate.get(), get()) == 0;
    }
