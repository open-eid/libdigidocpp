 ...
 *
 * For ASiC-E and ASiC-S containers, considers only signatures whose signing
 * certificate has country=EE; non-Estonian signatures are ignored when deciding
 * whether extension is possible. For ASiC-E containers with valid LT-level EE
 * signatures, extends eligible non-e-seal signatures in place by adding an
 * archive timestamp. For ASiC-S containers, re-timestamps the existing EE
 * timestamp token. If in-place extension is not possible, wraps the original
 * container as a data file in a new ASiC-S container and adds a timestamp.
 *
 * Containers where all considered EE signatures are T-level (time-stamp profile without OCSP)
 * cannot be extended and will throw. Containers with no Estonian signing certificates will throw.
 *
 ...
 * @param extendedCount set to the number of signatures extended in place; 0 if the container was wrapped.
 * @return new ASiC-S container if wrapping was necessary; empty unique_ptr if extended in place.
 * @throws Exception if the container has no signatures, no EE signatures, all signatures are T-level, or extension fails.
 * @since 4.5.0
 */
        ...
        doc.mediaType() == ASiContainer::MIMETYPE_ASIC_E)
    {
        ...
        for(Signature *s: doc.signatures())
        {
            ...
            if(signingCert.subjectName("C") != "EE" ||
                contains(signingCert.qcStatements(), digidoc::X509Cert::QCT_ESEAL))
                continue;
            ++eeCount;

            const string &prof = s->profile();
            if(prof.find(ASiC_S::ASIC_TST_PROFILE) != string::npos)
                break;
            if(prof.find(ASiC_E::ASIC_TS_PROFILE) == string::npos)
            {
                extendInPlace = false;
                break;
            }
            ...
            // Time-stamp signatures without OCSP cert are considered T-level signatures
            else if(auto ocspCert = s->OCSPCertificate(); !ocspCert)
            {
                ++tCount;
                continue;
            }
            else if(!ocspCert.isValid())
            {
                extendInPlace = false;
                break;
            }
        ...
        if(tCount > 0 && tCount == eeCount)
            THROW("Container contains only T-level signatures, cannot extend validity");
        if(eeCount == 0)
            THROW("Container does not contain signatures with Estonian signing certificate, cannot extend validity");
        ...
        if(extendInPlace)
        {
            for(Signature *s: doc.signatures())
            {
                if(auto cert = s->signingCertificate();
                    ...
                    contains(cert.qcStatements(), digidoc::X509Cert::QCT_ESEAL) ||
                    (s->profile().find(ASiC_E::ASIC_TS_PROFILE) != string::npos && !s->OCSPCertificate()))
                    continue;
