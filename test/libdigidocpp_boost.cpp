...
BOOST_AUTO_TEST_CASE(ThrowsWithNoSignatures)
{
    ...
    BOOST_CHECK_THROW(Container::extendContainerValidity(*d, &signer, extendedCount), Exception);
}
...
BOOST_AUTO_TEST_CASE(ExtendAsiceInPlace)
{
    ...
    size_t extendedCount{};
    BOOST_CHECK_NO_THROW(wrapped = Container::extendContainerValidity(*d, &signer, extendedCount));
    BOOST_CHECK(!wrapped); // nullptr = extended in place, no new container created
    BOOST_CHECK_EQUAL(extendedCount, 1U);
    BOOST_CHECK_EQUAL(d->signatures().size(), 1U);
    if(!d->signatures().empty())
        BOOST_CHECK_NO_THROW(d->signatures().front()->validate());
}
...
BOOST_AUTO_TEST_CASE(WrapContainerWithExpiredSignatures)
{
    ...
    BOOST_CHECK_EQUAL(extendedCount, 0U);
...
BOOST_AUTO_TEST_CASE(XMLTextValidation)
{
    ...
    for(string_view value: {
        string_view{"\0", 1},
        string_view{"\xC0\x80", 2},       // Overlong U+0000
        string_view{"\x80", 1},           // Isolated continuation byte
        string_view{"\xE2\x82", 2},       // Truncated sequence
        string_view{"\xF4\x90\x80\x80", 4}, // Above U+10FFFF
        string_view{"\xEF\xBF\xBE", 3},   // U+FFFE
        string_view{"\xEF\xBF\xBF", 3}    // U+FFFF
    })
